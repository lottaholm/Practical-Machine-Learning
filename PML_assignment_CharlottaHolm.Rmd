...
The out-of-sample-error is the error of the model when applied to the test set. For random forest models this is called the out-of-bag  (OOB) error. The estimate for the error can, for random forets models, be calculated directly from the models accuracy (An Introduction to Statistical Learning (2013), pp. 317-318, Gareth et.al., http://www-bcf.usc.edu/~gareth/ISL/):
```{r oob}
modFit$results[2] 
acc <- modFit$results[2]
```
Thus we can see that our random forest model run above has an OOB error rate of `r 1-mean(acc$Accuracy)`. 


## Predicting on the test set and checking whether the prediction was accurate:

We apply the model to the test set to see how well it predicts:
```{r predict, cache=TRUE}
pred <- predict(modFit, test)
table(pred, test$problem_id) ## Table to show the predicted values cross tabulated with the real values.
```
The table above shows us how many of each classe were predicted correctly.
